...
def create_aura_only(energy_values: Dict[str, float], width=500, height=600) -> np.ndarray:
    ...
    for y in range(height):
        for x in range(width):
            ...
            # Используем более плавные переходы между секциями
            if rel_y < 0.2:  # Ноги
                # Плавное расширение от ступней к бедрам
                body_width = 0.05 + rel_y * 0.5
            elif rel_y < 0.4:  # Нижняя часть туловища
                # Плавное расширение к торсу
                body_width = 0.15 + (rel_y - 0.2) * 0.75
                ...
                body_width = 0.3 - pow(rel_y - 0.6, 0.8) * 0.8
            else:  # Голова
                # Голова с плавным сужением к верху
                body_width = 0.15 - pow(rel_y - 0.8, 1.5) * 0.05
            
            # Рассчитываем горизонтальное расстояние от центра
            dx = (x - center_x) / width
            
            # Используем овальную форму вместо прямоугольной
            # Расстояние до эллипса: sqrt((x/a)^2 + (y/b)^2) = 1
            # где a и b - полуоси эллипса
            
            # Вычисляем координаты относительно центра фигуры по вертикали
            dy = (y - height * 0.5) / height
            
            # Рассчитываем нормализованное расстояние до тела (силуэта человека)
            # Используем формулу эллипса: (x/a)^2 + (y/b)^2 = 1
            # где x = dx, y = dy, a = body_width, b = 0.5 (половина высоты)
            
            # Для создания эллиптического силуэта используем формулу:
            rel_dist = ((dx / body_width) ** 2 + (dy * 2) ** 2) ** 0.5
            
            if rel_dist <= 1.0:
                # Внутри силуэта
                body_dist = 0
            else:
                # Абсолютное расстояние от границы силуэта
                body_dist = (rel_dist - 1.0) * width * body_width
            ...
            for chakra, pos_y in chakra_positions.items():
                ...
                if energy_factor > 0:
                    ...
                    if body_dist <= max_dist_for_chakra:
                        # Вычисляем расстояние от текущей точки до центра чакры
                        # Сначала находим горизонтальное положение чакры (предполагаем, что все чакры расположены на центральной линии)
                        chakra_x = center_x
                        
                        # Рассчитываем евклидово расстояние от точки до центра чакры
                        dx_to_chakra = x - chakra_x
                        dy_to_chakra = y - chakra_y
                        
                        # Расстояние от точки до центра чакры (нормализованное)
                        dist_to_chakra = ((dx_to_chakra / width) ** 2 + (dy_to_chakra / height) ** 2) ** 0.5
                        
                        # Используем нелинейное гауссово затухание вместо линейного
                        # Гауссова функция: exp(-dist^2 / (2*sigma^2))
                        # Где sigma определяет "радиус влияния" чакры
                        sigma = 0.3  # Определяет, насколько широко распространяется влияние чакры
                        
                        # Вес влияния чакры на эту точку (убывает с увеличением расстояния до чакры)
                        # Используем модифицированную гауссову функцию для более плавного перехода
                        angle_weight = math.exp(-dist_to_chakra ** 2 / (2 * sigma ** 2))
                        
                        # Горизонтальное влияние: убывает с расстоянием от тела
                        # Используем гладкую убывающую функцию вместо линейной
                        dist_factor = body_dist / max_dist_for_chakra
                        horiz_weight = math.exp(-3 * dist_factor)  # Экспоненциальное затухание
